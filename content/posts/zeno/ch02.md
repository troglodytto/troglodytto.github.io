...
Now let's create a new rust application.

I've chosen to call our OS **"Zeno"** (no particular reason, just sounds cool)
...
Great! the hardest part is done now.
...
By default, rust and its standard library depend heavily on OS abstractions to do a lot of heavylifting for us.

Rust's standard library assumes the presence of an underlying OS and the C runtime.

When you invoke the `println!` macro, rust makes syscalls to the host operating system to make stuff show up on your screen.

When you allocate something, Rust relies on the operating system to manage memory for it. i.e the interaction between your rust programs, and the physical RAM on your system

Most of what you do in rust, whether it be I/O, Memory, Syscalls, the whole lot, in one way or other rust relies on your operating system to do them.

However, in our case, **there is no OS**. We are writing our *own* operating system. Which means whatever rust relies on to do those aforementioned things, Gone!

What'd happen when rust tries to access them? Nothing good that's for sure, so we need to get rid of any references to those things.

Step 1. Tell rust to avoid using those things.

Add the `#![no_std]` attribute at the top of your `main.rs` file, and run it.
...
Only the `core` library remains. Everything else, that relies on an underlying OS is gone. `core` is platfor agnostic, so we can keep it.

```rust {title="main.rs"}
#![no_std] // 👈 Add this line
...
Hmmm. The [Rustnomicon](https://doc.rust-lang.org/nomicon/panic-handler.html) says


>#[panic_handler] is used to define the behavior of panic! in #![no_std] applications. The #[panic_handler] attribute must be applied to a function with signature **fn(&PanicInfo) -> !** and such function must appear once in the dependency graph of a binary / dylib / cdylib crate.
...
use core::panic::PanicInfo; // 👈 Add this line

fn main() {}


// 👇 Add this function (notice the `#[panic_handler]` attribute)
#[panic_handler]
...
Additionally, another one of the default behaviors of the rust compiler is to unwind the stack whenever a program panics. This is done to allow cleanup of resources like memory, files, etc by calling `Drop::drop` methods on each stack frame. We've disabled that behavior by setting `panic = "abort"` in our `Cargo.toml` because a panic in *our* kernel means immediate shutdown i.e aborting the entire kernel.

To fix this, we also need to add the following in our `Cargo.toml` to tell rust that we are no longer using `unwinding` panics.

```toml {title="Cargo.toml"}
[profile.dev] # 👈 For dev mode
panic = "abort"

[profile.release] # 👈 For release mode
panic = "abort"
```
...
We get a hint of that when we try to compile in the form of a giant cryptic error.
...
When you hear the C "runtime", its not a runtime in the traditional sense as in, there's no VM or an interpreter that runs your programs, instead runtime in this case basically means that the `main` gets wired to the `crt0` startup routines.
...
void _start() {
    ...
    // 👇 Call to the main function that you wrote
    int exit_code = main(
        argc, 
        argv, 
        envp
...
When you start a program, it doesn't actually jump to the `main` function but instead, it jumps to a symbol called `_start` (in most cases, although it could totally depend on your ELF loader)

In our case, we need to tell the linker to *not* include the C runtime.
