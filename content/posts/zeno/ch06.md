...
In the [last post]({{< ref "/posts/zeno/ch05.md" >}}), we discussed how to set up serial output for our operating system. We setup a `dbg!` as a debug mechanism.
...
For that, we need to be able to read and write to specific locations in physical memory but since as discussed in the **[Integrating a concrete, battle tested bootloader]({{< ref "/posts/zeno/ch04.md" >}})** post, x86 long mode simply does not work without paging.
