...
That we sector read is called the [**Master Boot Record**](https://en.wikipedia.org/wiki/Master_boot_record) and is exactly 512 bytes (Disks are usually divided into 512 byte sectors)
...
The firmware first checks if the last 2 bytes (at address `0x1fe` and `0x1ff`) of the MBR is equal to `0x55aa`
