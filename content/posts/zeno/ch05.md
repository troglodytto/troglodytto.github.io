...
In the [last post]({{< ref "/posts/zeno/ch04.md" >}}), we prepared the foundations for our operating system. We set up a basic build system using Cargo and Rust, and we created a simple bootloader to load our kernel into memory.
...
// 👇 This allows us to treat the serial port as a Writable target
...
fn main() {
    ...
    // 👇 instead of manually iterating the bytes in our string and writing them out one by one
-   for &character in b"Hello, world!" {
-       serial_port.send(character);
-   }
...
We'll follow the same structure we defined in the [**housekeeping**]({{< ref "/posts/zeno/housekeeping.md" >}}) chapter.
...
[Next: Foundations of memory management]({{< ref "/posts/zeno/ch06.md" >}})
